...
class RedBlackTree():
    ...
    def is_red(self, my_node):
        # Метод проверяет, есть ли у данной ноды ненулевое значение, а затем и ее цвет равен ли он красному
    ...
    def insert(self, data):
        # Метод вставляет ноду с указанным значением в дерево. Если нет корневой ноды, то нода создается и устанавливается как корневая
        node = None
        if self.root:
    ...
    def balanced(self, my_node):
        # Метод проверяет, есть ли у ноды левый или правый указатель после чего зависит от этого вызывается ли метод balanced
